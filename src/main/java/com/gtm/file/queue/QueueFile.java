...
public class QueueFile {
    ...
    private void ringErase(int position, int length) throws IOException {
        while(length > 0) {
            int chunk = Math.min(length, ZEROES.length);
    ...
    public synchronized void remove() throws IOException {
        if (this.isEmpty()) {
            ...
            if (this.elementCount == 1) {
                ...
                int firstTotalLength = 4 + this.first.length;
                this.ringErase(this.first.position, firstTotalLength);
                int newFirstPosition = this.wrapPosition(this.first.position + firstTotalLength);
